...
std::wstring common::utility::stringhelper::Int64ToString( __int64 value )
{
	// log10(2) ~= 0.3 bytes needed per bit or per byte log10(2**8) ~= 2.4.
	// So round up to allocate 3 output characters per byte, plus 1 for '-'.
	const int kOutputBufSize = 3 * sizeof(__int64) + 1;
	...
	// unsigned, even the presence of the unary operation causes a warning. 
	// eg. 0x80-00-00-00, is negative when param in unsigned  passed
	unsigned __int64 res = (is_neg) ? static_cast<unsigned __int64>(-value) : static_cast<unsigned __int64>(value);
...
//ansi版本
bool common::utility::stringhelper::StringToIntA(const std::string& in_str, int* iRetInt)
{
	assert(iRetInt != NULL);
	if (iRetInt == NULL)
	{
...
//ansi版本
int common::utility::stringhelper::StringToIntA(const std::string& in_str)
{
	int i=0;

	bool bRet = StringToIntA(in_str, (int*)&i );
	if( bRet == true)
		return i;
...
//wchar版本
bool common::utility::stringhelper::StringToInt(const std::wstring& in_str, int* iRetInt)
{
	assert(iRetInt != NULL);
	if (iRetInt == NULL)
	{
...
//wchar版本
int common::utility::stringhelper::StringToInt(const std::wstring& in_str)
{
	int i=0;

	bool bRet = StringToInt(in_str, (int*)&i );
	if( bRet == true)
		return i;
...
std::string common::utility::stringhelper::HexEncodeCopy(const std::string& in_str)
{
	...
	std::string ret(iSize*2+1, '\0'); //\0结束
